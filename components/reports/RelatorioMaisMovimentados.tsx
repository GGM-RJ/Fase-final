...
const RelatorioMaisMovimentados: React.FC<RelatorioMaisMovimentadosProps> = ({ transferHistory }) => {
  ...
  const allQuintas = ['Todas', ...quintaService.getQuintas().map(q => q.name), 'Stock Geral', 'Consumo'];
                        ...
                        {rankedWines.map((wine, index) => (
                            <tr key={wine.key} className="bg-white border-b hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 font-medium">
                                    <div className="text-gray-800">{wine.brand}</div>
