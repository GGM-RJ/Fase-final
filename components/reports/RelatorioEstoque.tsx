...
const RelatorioEstoque: React.FC<RelatorioEstoqueProps> = ({ stock }) => {
  ...
  const locations = useMemo(() => {
    const quintas = quintaService.getQuintas().map(q => q.name);
    return ['Todas', 'Stock Geral', ...quintas];
  }, []);
  ...
  const reportData = useMemo<{ [key: string]: StockItem[] }>(() => {
    ...
    if (selectedLocation === 'Todas') {
        groups['Stock Geral'] = [];
        quintaService.getQuintas().forEach(q => groups[q.name] = []);
    } else {
  ...
  const generateReportContent = () => {
    ...
    Object.entries(reportData).forEach(([location, items]: [string, StockItem[]]) => {
        // If "Todas" is selected, skip empty locations if desired, or keep them to show they are empty.
        // Let's show them but indicate if empty.
        ...
        if (items.length > 0) {
            ...
            tableRows = `<tr><td colspan="4" style="text-align: center; color: #888;">Sem stock nesta localização.</td></tr>`;
        }
    ...
    <div>
        ...
        <div className="space-y-8">
            {Object.entries(reportData).map(([location, items]: [string, StockItem[]]) => {
                ...
                if (items.length === 0 && selectedLocation === 'Todas') {
                     // Optional: Skip rendering empty sections in 'Todas' view if preferred
                     // return null; 
                }
                                    ...
                                    <tbody>
                                        {items.map(item => (
                                            <tr key={item.id} className="border-b hover:bg-gray-50 last:border-b-0">
                                                <td className="px-6 py-3 font-medium">{item.brand}</td>
