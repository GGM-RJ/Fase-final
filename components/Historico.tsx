...
const Historico: React.FC<HistoricoProps> = ({ transferHistory }) => {
    ...
    const allQuintas = useMemo(() => {
        return ['Todas', ...quintaService.getQuintas().map(q => q.name), 'Consumo'];
    }, []);
    ...
    const generatePrintContent = (logs: TransferLog[]) => {
        const tableRows = logs.map(log => {
            ...
            if (log.status === 'Aprovado' && log.approverName) {
                statusCell += `<br><small style="font-size: 10px; color: #555;">por ${log.approverName}</small>`;
            }
            return `
                <tr>
                        ...
                        {sortedHistory.length > 0 ? sortedHistory.map(log => (
                            <tr key={log.id} className="bg-white border-b hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 text-center">
                                    ...
                                    {log.status === 'Aprovado' && log.approverName && (
                                        <div className="text-xs text-gray-500 mt-1">por {log.approverName}</div>
                                    )}
                                </td>
                            </tr>
                        )) : (
